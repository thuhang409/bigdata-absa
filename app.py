...
all_attrs = [['GENERAL', 'PRICE', 'QUALITY'],
             ['SERVICE', 'FOODSTYLES', 'LOCATION', 'BACKGROUND']]
...
@st.cache
def combine_entity_attr(aspects):
    return [f'{aspect}' for aspect in aspects]


all_keys = combine_entity_attr(aspects)
...
def txt2df(uploaded_file):
    ...
    df = pd.DataFrame()
    df['review'] = [review for review in data[2::4]]
    df[all_keys] = [label_encoder(label) for label in data[3::4]]
...
if st.session_state.mode == modes[0]:
    ...
    uploaded_file = st.sidebar.file_uploader('Upload texts', type='csv')
    ...
    if st.session_state.df is not None:
        ...
        df = convert_csv(st.session_state.df)
    ...
    def on_change_sentiment(key):
        ...
        doc_id = state.doc_id
        df = state.df
        col_index = all_keys.index(key) + 2
    ...
    def auto():
        ...
        result = classify_sentence(text)
        df.iloc[doc_id, 2:] = [sentiments[result[i]]
    ...
    if df1 is not None and df2 is not None and df3 is not None:
        if (len(df1) != len(df2) != len(df3)
            ...
            y1 = df1.iloc[:, 2:].replace(mapping).astype(np.uint8).values
            y2 = df2.iloc[:, 2:].replace(mapping).astype(np.uint8).values
            y3 = df3.iloc[:, 2:].replace(mapping).astype(np.uint8).values
